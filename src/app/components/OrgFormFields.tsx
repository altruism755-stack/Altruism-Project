...
export interface OrgFormFieldsProps {
  ...
  /**
   * When provided, only field blocks whose keys intersect this list are rendered.
   * Use to split the form across multiple section cards without duplicating logic.
   * Omit to render all fields (default behaviour on RegisterPage).
   */
...
export function OrgFormFields(props: OrgFormFieldsProps) {
  const { state, errors, touched, focused,
    ...
    lockedFields, mode = "register", showSubmitter = true,
    sectionFilter } = props;

  const isLocked = (f: string) => !!lockedFields?.has(f);

  // Returns true when the block should be rendered.
  // A block passes if sectionFilter is absent OR any of its field keys are in the filter.
  const show = (keys: string[]) =>
    !sectionFilter || keys.some((k) => sectionFilter.includes(k));
  ...
  return (
    <>
      ...
      {/* Type + Founded Year — rendered together only when both are in scope */}
      {show(["orgType", "foundedYear"]) && (
      ...
      {/* HQ — always render as a unit; omit entirely if neither key is in scope */}
      {show(["hqGovernorate", "hqCity"]) && (
      ...
      {/* Logo — lock key is "logo"; state key is "logoDataUri"; accept both in filter */}
      {show(["logo", "logoDataUri"]) && (
        <div>
      ...
      {showSubmitter && show(["submitterName", "submitterRole", "additionalNotes"]) && (
        <>
